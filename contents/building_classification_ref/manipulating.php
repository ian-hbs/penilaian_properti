<?php
	...
	if($act=='add' || $act=='edit')
	{
		$arr_field = array('id_klasifikasi_bangunan','klasifikasi_bangunan');
		
		foreach($_POST as $key => $val)
		{
			if(in_array($key,$arr_field))
			{
				if($key=='klasifikasi_bangunan')
	...
	if($act=='add')
	{
		...
		$activity = "menambah data ke table ref_klasifikasi_bangunan (id_klasifikasi_bangunan=".$arr_data['id_klasifikasi_bangunan'].")";
	...
	{
		$id = $_POST['id_'];
		$cond = "id_klasifikasi_bangunan='".$id."'";
		$result = $DML->update($arr_data,$cond);		
		
		if(!$result)
			die('failed');

		$activity = "merubah data pada table ref_klasifikasi_bangunan (id_klasifikasi_bangunan=".$id.")";
		$global->insert_logs($activity,$ip);
	}
	else if($act=='delete')
	{
		...
		$activity = "menghapus data dari table ref_klasifikasi_bangunan (id_klasifikasi_bangunan=".$id.")";
		$global->insert_logs($activity,$ip);
	}	
