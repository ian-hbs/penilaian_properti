<?php
	...
	if($act=='add' || $act=='edit')
	{
		$arr_field = array('id_kelompok_komponen_bangunan','kelompok_komponen_bangunan','diklasifikasi','maks_inputan');
		
		foreach($_POST as $key => $val)
		{
			if(in_array($key,$arr_field))
			{
				if($key=='kelompok_komponen_bangunan')
	...
	if($act=='add')
	{
		...
		$activity = "menambah data ke table ref_kelompok_komponen_bangunan (id_kelompok_komponen_bangunan=".$arr_data['id_kelompok_komponen_bangunan'].")";
	...
	{
		$id = $_POST['id_'];
		$cond = "id_kelompok_komponen_bangunan='".$id."'";
		$result = $DML->update($arr_data,$cond);		
		
		if(!$result)
			die('failed');

		$activity = "merubah data pada table ref_kelompok_komponen_bangunan (id_kelompok_komponen_bangunan=".$id.")";
		$global->insert_logs($activity,$ip);
	}
	else if($act=='delete')
	{
		...
		$activity = "menghapus data dari table ref_kelompok_komponen_bangunan (id_kelompok_komponen_bangunan=".$id.")";
		$global->insert_logs($activity,$ip);
	}	
