<?php
	...
	$DML = new DML('ref_nilai_komponen_bangunan',$db);
	...
	if($act=='add')
	{
		$id_nilai_komponen_bangunan = $global->get_incrementID('ref_nilai_komponen_bangunan','id_nilai_komponen_bangunan');
		$arr_data['id_nilai_komponen_bangunan'] = $id_nilai_komponen_bangunan;
		$result = $DML->save($arr_data);
				
		if(!$result)
			die('failed');

		$activity = "menambah data ke table ref_nilai_komponen_bangunan (id_nilai_komponen_bangunan=".$arr_data['id_nilai_komponen_bangunan'].")";
	...
	{
		$id = $_POST['id_'];
		$cond = "id_nilai_komponen_bangunan='".$id."'";
		$result = $DML->update($arr_data,$cond);		
		
		if(!$result)
			die('failed');

		$activity = "merubah data pada table ref_nilai_komponen_bangunan (id_nilai_komponen_bangunan=".$id.")";
		$global->insert_logs($activity,$ip);
	}
	else if($act=='delete')
	{
		...
		$activity = "menghapus data dari table ref_nilai_komponen_bangunan (id_nilai_komponen_bangunan=".$id.")";
		$global->insert_logs($activity,$ip);
	}	
