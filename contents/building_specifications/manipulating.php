<?php
	...
	$arr_field = array('fk_penugasan','pondasi','dinding','lantai','dinding_dalam','dinding_luar','kusen','atap','pagar');
	...
	if($act=='add')
	{
		$id_spesifikasi_bangunan = $global->get_incrementID('spesifikasi_bangunan','id_spesifikasi_bangunan');
		$arr_data['id_spesifikasi_bangunan'] = $id_spesifikasi_bangunan;
		$result = $DML->save($arr_data);
				
		if(!$result)
			die('failed');

		$activity = "menambah data ke tabel spesifikasi_bangunan (fk_penugasan=".$arr_data['fk_penugasan'].")";
	...
	else if($act=='edit')
	{
		...
		$activity = "merubah data pada tabel spesifikasi_bangunan (fk_penugasan=".$fk_penugasan.")";
		$global->insert_logs($activity,$ip);
	}
