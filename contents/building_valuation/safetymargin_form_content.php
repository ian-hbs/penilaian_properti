<?php
    ...
    // fetching perhitungan_bangunan
    $sql = "SELECT SUM(market_value) as nilai_pasar_bangunan FROM perhitungan_bangunan WHERE(fk_penugasan='".$id_penugasan."') AND (type='".($jenis_objek=='bangunan'?'building':'site improvement')."')";
	$result = $db->Execute($sql);
	if(!$result)
		echo $db->ErrorMsg();

	$npb = 0;
	$ndpb = $result->RecordCount();
	if($ndpb>0)
	{
		$dpb = $result->FetchRow();
		$npb = $dpb['nilai_pasar_bangunan'];
	}	
	$disabled = ($ndpb==0?'disabled':'');
	// == //
    ...
    $sql = "SELECT id_nilai_safetymargin,total_score,prosentase,nilai FROM nilai_safetymargin WHERE(fk_penugasan='".$id_penugasan."') AND (jenis_objek='".$jenis_objek."')";
...
<script type="text/javascript">
    ...
    function get_safetymargin()
    {
    	...
    	var npt = $('#nilai_pasar_bangunan').val();
...
<?php
    ...
    <form class='form-horizontal' id='".$form_id."' method='POST' action='contents/".$fn."/safetymargin_manipulating.php'>
        ...
        <input type='hidden' name='jenis_objek' value='".$jenis_objek."'/>
        <div class='row'>
            <div class='col-md-12'>";
                
                $lbl = ($jenis_objek=='bangunan'?'Bangunan':'Sarana Pelengkap');;
            	...
            	if($ndpb==0)
            	{
	            	...
	            	<div class='alert alert-warning' role='alert'>
		              ...
		              Silahkan lakukan perhitungan Nilai ".$lbl." terlebih dahulu!
		            </div>";
		        }
            	
            	echo "
            	<div class='form-group'>
            		<label class='col-sm-4 control-label'>Nilai Pasar ".$lbl."</label>
            		<div class='col-sm-3'>
            			<input type='text' class='form-control readonly-bg' id='nilai_pasar_bangunan' name='nilai_pasar_bangunan' style='text-align:right' value='".number_format($npb)."' readonly/>
            		</div>
            	</div>";
            	
            	$data = $DML1->fetchData("SELECT id_param_safetymargin,deskripsi FROM ref_param_safetymargin WHERE(jenis_objek='".$jenis_objek."')");
            	$no = 0;
