<?php
    ...
    $DML = new DML('ref_jenis_sertifikat',$db);

    $_SYSTEM_PARAMS = $_APP_PARAM['system_params'];

    $id_name = 'id_jenis_sertifikat';
    $id_value = ($act=='edit'?$_POST['id']:'');
    $arr_field = array('jenis_sertifikat','akronim');
    $curr_data = $DML->getCurrentData($act,$arr_field,$id_name,$id_value);
    $id_form = 'certificate-type-form';
?>
...
<form id="<?=$id_form?>" class="form-horizontal form-label-left" method="POST" action="contents/<?=$fn?>/manipulating.php">
    ...
    <div class="form-group">
        <label class="control-label col-md-4 col-sm-4 col-xs-12" for="<?=$id_name?>">ID Sertifikat <font color="red">*</font></label>
    ...
    <div class="form-group">
        <label class="control-label col-md-4 col-sm-4 col-xs-12" for="jenis_sertifikat">Jenis Sertifikat <font color="red">*</font></label>
        <div class="col-md-8 col-sm-8 col-xs-12">
            <input type="text" id="jenis_sertifikat" name="jenis_sertifikat" value="<?=$curr_data['jenis_sertifikat']?>" class="form-control col-md-7 col-xs-12" required>
        </div>
    </div>
    <div class="form-group">
        <label class="control-label col-md-4 col-sm-4 col-xs-12" for="akronim">Akronim <font color="red">*</font></label>
        <div class="col-md-8 col-sm-8 col-xs-12">
            <input type="text" id="akronim" name="akronim" value="<?=$curr_data['akronim']?>" class="form-control col-md-7 col-xs-12" required>
        </div>
    </div>    
