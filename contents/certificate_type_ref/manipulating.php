<?php
	...
	if($act=='add' || $act=='edit')
	{
		$arr_field = array('id_jenis_sertifikat','jenis_sertifikat','akronim');
		
		foreach($_POST as $key => $val)
		{
			if(in_array($key,$arr_field))
			{
				if($key=='jenis_sertifikat' or $key=='akronim')
	...
	if($act=='add')
	{
		...
		$activity = "menambah data ke table ref_jenis_sertifikat (id_jenis_sertifikat=".$arr_data['id_jenis_sertifikat'].")";
	...
	{
		$id = $_POST['id'];
		$cond = "id_jenis_sertifikat='".$id."'";
		$result = $DML->update($arr_data,$cond);		
		
		if(!$result)
			die('failed');

		$activity = "merubah data pada table ref_jenis_sertifikat (id_jenis_sertifikat=".$id.")";
		$global->insert_logs($activity,$ip);
	}
	else if($act=='delete')
	{
		...
		$activity = "menghapus data dari table ref_jenis_sertifikat (id_jenis_sertifikat=".$id.")";
		$global->insert_logs($activity,$ip);
	}	
