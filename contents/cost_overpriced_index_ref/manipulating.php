<?php
	...
	$DML = new DML('ref_indeks_kemahalan_konstruksi',$db);
	...
	if($act=='add')
	{
		$id = $global->get_incrementID('ref_indeks_kemahalan_konstruksi','id_indeks');
		$arr_data['id_indeks'] = $id;
		$result = $DML->save($arr_data);
				
		if(!$result)
			die('failed');

		$activity = "menambah data ke table ref_indeks_kemahalan_konstruksi (id_indeks=".$arr_data['id_indeks'].")";
	...
	else if($act=='edit')
	{
		...
		$activity = "merubah data pada table ref_indeks_kemahalan_konstruksi (id_indeks=".$id.")";
		$global->insert_logs($activity,$ip);
	}
	else if($act=='delete')
	{
		...
		$activity = "menghapus data dari table ref_indeks_kemahalan_konstruksi (id_indeks=".$id.")";
		$global->insert_logs($activity,$ip);
	}	
