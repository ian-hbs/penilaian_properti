<?php
	...
	if($act=='add')
	{
		$id = $global->get_incrementID('ref_nilai_bahan_bangunan','id_nilai_bahan');
		$arr_data['id_nilai_bahan'] = $id;
		$result = $DML->save($arr_data);
				
		if(!$result)
			die('failed');

		$activity = "menambah data ke table ref_nilai_bahan_bangunan (id_nilai_bahan=".$arr_data['id_nilai_bahan'].")";
	...
	{
		$id = $_POST['id'];
		$cond = "id_nilai_bahan='".$id."'";
		$result = $DML->update($arr_data,$cond);		
		
		if(!$result)
			die('failed');

		$activity = "merubah data pada table ref_nilai_bahan_bangunan (id_nilai_bahan=".$id.")";
		$global->insert_logs($activity,$ip);
	}
	else if($act=='delete')
	{
		...
		$activity = "menghapus data dari table ref_nilai_bahan_bangunan (id_nilai_bahan=".$id.")";
		$global->insert_logs($activity,$ip);
	}	
