<?php
	...
	if($act=='add' || $act=='edit')
	{
		$arr_field = array('id_jenis_objek','jenis_objek');
	...
	if($act=='add')
	{
		...
		$activity = "menambah data ke table ref_jenis_objek (id_jenis_objek=".$arr_data['id_jenis_objek'].")";
	...
	{
		$id = $_POST['id'];
		$cond = "id_jenis_objek='".$id."'";
		$result = $DML->update($arr_data,$cond);		
		
		if(!$result)
			die('failed');

		$activity = "merubah data pada table ref_jenis_objek (id_jenis_objek=".$id.")";
		$global->insert_logs($activity,$ip);
	}
	else if($act=='delete')
	{
		...
		$activity = "menghapus data dari table ref_jenis_objek (id_jenis_objek=".$id.")";
		$global->insert_logs($activity,$ip);
	}	
