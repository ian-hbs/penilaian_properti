<?php
	...
	if($act=='add' || $act=='edit')
	{
		$arr_field = array('id_perusahaan_penunjuk','perusahaan_penunjuk','kantor_cabang','alamat','kota','kode_pos','no_kerjasama','tgl_kerjasama','jenis');
		
		foreach($_POST as $key => $val)
		{
			if(in_array($key,$arr_field))
			{
				if($key=='tgl_kerjasama')
	...
	if($act=='add')
	{
		...
		$activity = "menambah data ke table ref_perusahaan_penunjuk (id_perusahaan_penunjuk=".$arr_data['id_perusahaan_penunjuk'].")";
	...
	{
		$id = $_POST['id'];
		$cond = "id_perusahaan_penunjuk='".$id."'";
		$result = $DML->update($arr_data,$cond);		
		
		if(!$result)
			die('failed');

		$activity = "merubah data pada table ref_perusahaan_penunjuk (id_perusahaan_penunjuk=".$id.")";
		$global->insert_logs($activity,$ip);
	}
	else if($act=='delete')
	{
		...
		$activity = "menghapus data dari table ref_perusahaan_penunjuk (id_perusahaan_penunjuk=".$id.")";
		$global->insert_logs($activity,$ip);
	}	
