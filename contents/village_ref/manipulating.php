<?php
	...
	if($act=='add' || $act=='edit')
	{
		$arr_field = array('id','district_id','name','office_address','phone_number','postal_code');
	...
	if($act=='add')
	{
		...
		$activity = "menambah data ke table ref_villages (id=".$arr_data['id'].")";
	...
	else if($act=='edit')
	{
		...
		$activity = "merubah data pada table ref_villages (id=".$arr_data['id'].")";
		$global->insert_logs($activity,$ip);
	}
	else if($act=='delete')
	{
		...
		$activity = "menghapus data dari table ref_villages (id=".$id.")";
		$global->insert_logs($activity,$ip);
	}	
